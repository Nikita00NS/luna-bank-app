...
export default function ExchangeScreen() {
  ...
  const execute = () => {
    ...
    const order = { id: uid(), coin: selCoin.sym, side, amount: val, price, total: totalLNC, time: new Date().toISOString() };
    setOrders(prev => [order, ...prev]);
    addTx({ id: order.id, from_user_id: user.telegram_id, to_user_id: user.telegram_id, from_account_id: side === 'buy' ? lncAcc.id : (targetAcc?.id || ''), to_account_id: side === 'buy' ? (targetAcc?.id || '') : lncAcc.id, amount: val, fee: 0, currency: selCoin.sym as any, type: 'transfer', status: 'completed', note: `${side.toUpperCase()} ${selCoin.sym}`, created_at: new Date().toISOString() });
    setAmount(''); setSelCoin(null); setTab('orders');
  };
      ...
      <div className="px-5 flex gap-2 mb-3">
        {(['market', 'trade', 'orders'] as const).map(t => (
          <button key={t} onClick={() => setTab(t)} className={`flex-1 py-2 rounded-xl text-sm font-semibold transition-all ${tab === t ? 'bg-white text-black' : 'glass text-white/50'}`}>
            {t === 'market' ? '📈 Рынок' : t === 'trade' ? '💹 Торговля' : `📋 Ордера (${orders.length})`}
          </button>
        ))}
      </div>
      <div className="flex-1 overflow-y-auto px-5 pb-24">
            ...
            {COINS.map((coin, i) => {
              ...
              return (
                <button key={coin.sym} onClick={() => { setSelCoin(coin); setTab('trade'); haptic('light'); }}
                  className="w-full glass p-4 flex items-center gap-4 animate-slide-up active:scale-[0.98] transition-all"
        ...
        {tab === 'trade' && (
          <div className="animate-fade-in">
            {!selCoin ? <div className="text-center py-16"><p className="text-white/35">Выберите монету на вкладке «Рынок»</p></div> : (
              <>
                <div className="glass-accent p-4 mb-4">
                ...
                {val > 0 && <div className="glass p-3 mb-4"><div className="flex justify-between text-sm"><span className="text-white/35">{side === 'buy' ? 'К оплате' : 'Получите'}</span><span className="mono font-bold">◎ {totalLNC.toFixed(2)} LNC</span></div></div>}
                <button onClick={execute} disabled={!val} className={`w-full py-4 rounded-2xl font-bold text-base transition-all active:scale-[0.97] ${side === 'buy' ? 'bg-emerald-500 text-white' : 'bg-red-500 text-white'} disabled:opacity-30`}>
        ...
        {tab === 'orders' && (
          <div className="animate-fade-in">
            {orders.length === 0 ? <div className="text-center py-16"><p className="text-white/35">Нет ордеров</p></div> : (
              <div className="space-y-2">{orders.map((o, i) => (
                <div key={o.id} className="glass p-3 flex items-center gap-3 animate-slide-up" style={{ animationDelay: `${i * 0.05}s` }}>
