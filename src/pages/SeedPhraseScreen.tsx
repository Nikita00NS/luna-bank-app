...
const WORDLIST = [
  ...
  'cease', 'celebrate', 'certain', 'chair', 'chalk', 'champion', 'change', 'chaos', 'chapter', 'charge',
...
export default function SeedPhraseScreen() {
  const { go, back, setTonWallet } = useStore();
  const [tonConnectUI] = useTonConnectUI();
  const [tab, setTab] = useState<'create' | 'import' | 'show'>('create');
  ...
  const [step, setStep] = useState<'intro' | 'write' | 'confirm' | 'done'>('intro');
  ...
  const handleWordInput = (val: string) => {
    setWordInput(val.toLowerCase());
    if (val.length > 0) {
      setSuggestions(WORDLIST.filter(w => w.startsWith(val.toLowerCase())).slice(0, 5));
    } else {
      setSuggestions([]);
    }
  };
  ...
  const handleImport = () => {
    ...
    if (words.some(w => !w) || words.length < 12) {
      toastError('Enter all 12 words');
      return;
    }
    for (const w of words) {
      if (!WORDLIST.includes(w.toLowerCase())) {
        toastError(`Word "${w}" not in BIP39 wordlist`);
        return;
      }
    }
    setTonWallet(`EQ${Array.from({length: 46}, () => 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789'[Math.floor(Math.random() * 62)]).join('')}`);
  ...
  const handleConfirmDone = () => {
    setTonWallet(`EQ${Array.from({length: 46}, () => 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789'[Math.floor(Math.random() * 62)]).join('')}`);
    toastSuccess('Wallet created! Seed phrase saved');
    haptic('success');
    go('home');
  };
  ...
  return (
    <div className="page safe-top">
      <div className="header">
        ...
        <p className="header-title">Seed Phrase</p>
      </div>

      <div className="flex gap-1.5 px-4 mt-2">
        {(['create', 'import', 'show'] as const).map(t => (
          <button key={t} onClick={() => { setTab(t); setStep('intro'); haptic('light'); }}
            ...
            {t === 'create' ? 'Create' : t === 'import' ? 'Import' : 'View'}
          </button>
        ))}
      </div>

      <div className="px-4 mt-4">
        {tab === 'create' && step === 'intro' && (
          <div className="text-center">
            <div className="w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-4 bg-[var(--accent)/10]">
            ...
            <p className="text-sm text-[var(--text-tertiary)]">
              A 12-word seed phrase will be generated. Write it down and store securely.
              Never share your seed phrase with anyone!
            </p>
            <div className="card p-4 mt-4 text-left text-xs space-y-2 bg-[var(--orange)/10]">
              <p className="text-[var(--orange)] font-medium">Important rules:</p>
              <ul className="list-disc pl-4 space-y-1 text-[var(--text-secondary)]">
                <li>Never share your seed phrase</li>
                <li>Store in a safe place (not on phone)</li>
                <li>Without seed phrase you cannot recover access</li>
                <li>Luna Wallet does not store your seed phrase</li>
              </ul>
            </div>
            <button onClick={generatePhrase} className="btn btn-primary w-full mt-6">
              Create Wallet
            </button>
          </div>
        )}

        {tab === 'create' && step === 'write' && (
          <div>
            <h3 className="font-semibold text-sm mb-3">Write down these 12 words in order:</h3>
            ...
            <button onClick={() => { navigator.clipboard.writeText(generatedPhrase.join(' ')); toastSuccess('Copied'); haptic('light'); }}
              className="btn btn-secondary w-full mt-3 flex items-center justify-center gap-2">
              <CopyIcon size={16} color="var(--text-secondary)" /> Copy
            </button>
            <button onClick={handleCreateConfirm} className="btn btn-primary w-full mt-3">
        ...
        {tab === 'create' && step === 'confirm' && (
          <div>
            <h3 className="font-semibold text-sm mb-3">Confirm seed phrase</h3>
            <p className="text-xs text-[var(--text-tertiary)] mb-3">
              Enter words in order to confirm
            </p>
            <div className="space-y-2">
              {generatedPhrase.map((word, i) => (
                <div key={i} className="flex items-center gap-2">
                  <span className="text-xs font-mono w-6 text-[var(--text-tertiary)]">{i + 1}.</span>
                  <input
                    type="text"
                    value={words[i] || ''}
                    onChange={(e) => {
                      const nw = [...words];
                      nw[i] = e.target.value.toLowerCase();
                      setWords(nw);
                    }}
                    placeholder="..."
                    className="input flex-1 text-sm" />
                  {words[i] === word && <span className="text-[var(--green)]">✓</span>}
                  {words[i] && words[i] !== word && <span className="text-[var(--red)]">✗</span>}
                </div>
              ))}
            </div>
            <button onClick={handleConfirmDone}
              disabled={generatedPhrase.some((w, i) => words[i] !== w)}
        ...
        {tab === 'import' && (
          <div>
            <div className="card p-4 mb-4 text-sm">
              <p className="font-medium mb-1">Import Wallet</p>
              <p className="text-[var(--text-tertiary)]">Enter 12 seed phrase words in order</p>
            </div>

            <div className="relative mb-3">
              <div className="flex items-center gap-2">
                <span className="text-xs font-mono text-[var(--text-tertiary)]">#{currentWordIndex + 1}</span>
                <input
                  type="text"
                  value={wordInput}
                  ...
                  onKeyDown={(e) => {
                    ...
                    if (e.key === 'Backspace' && !wordInput && currentWordIndex > 0) {
                      setCurrentWordIndex(currentWordIndex - 1);
                      setWordInput(words[currentWordIndex - 1] || '');
                    }
                  }}
              ...
              {suggestions.length > 0 && (
                <div className="absolute z-10 left-0 right-0 mt-1 rounded-lg overflow-hidden border border-[var(--border)] bg-[var(--bg-card)]">
                  {suggestions.map(s => (
                    <button key={s} onClick={() => selectWord(s)}
                      className="w-full px-4 py-2 text-left text-sm active:bg-[var(--bg-card-hover)]">
                      {s}
                    </button>
                  ))}
                </div>
            ...
            <div className="flex flex-wrap gap-1.5 mb-4">
              {words.map((w, i) => (
                <button key={i} onClick={() => { setCurrentWordIndex(i); setWordInput(w); }}
                  className={`px-2.5 py-1 rounded-lg text-[10px] font-medium transition-all ${i === currentWordIndex ? 'ring-2 ring-[var(--accent)]' : ''}`}
                  style={{
                    background: w ? 'var(--accent)/10' : 'var(--bg-card)',
                    color: w ? 'var(--accent)' : 'var(--text-tertiary)',
                  }}>
                  {i + 1}. {w || '...'}
                </button>
              ))}
            ...
            <div className="flex flex-wrap gap-1 max-h-40 overflow-y-auto mb-4">
              {WORDLIST.slice(0, 100).map(w => (
                <button key={w} onClick={() => { selectWord(w); }}
                  className="px-2 py-0.5 rounded text-[9px] active:bg-[var(--bg-card-hover)] text-[var(--text-tertiary)]">
                  {w}
                </button>
              ))}
            </div>
        ...
        {tab === 'show' && (
          <div className="text-center">
            <div className="w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-4 bg-[var(--red)/10]">
            ...
            <h2 className="text-xl font-bold mb-2">Seed phrase not stored</h2>
            <p className="text-sm text-[var(--text-tertiary)]">
              For security, your seed phrase is not stored in this app.
              If you created wallet via TON Connect, seed phrase is in your wallet app (Tonkeeper, etc.)
            </p>
            <div className="card p-4 mt-4 text-left text-xs bg-[var(--orange)/10]">
              <p className="text-[var(--orange)] font-medium">How to recover wallet:</p>
              <ol className="list-decimal pl-4 mt-2 space-y-1 text-[var(--text-secondary)]">
                <li>Open Tonkeeper (or other wallet)</li>
                <li>Go to Settings → Security</li>
                <li>Select "Show seed phrase"</li>
                <li>Enter wallet password</li>
              </ol>
            </div>
          </div>
        )}
