...
import { haptic, formatCrypto, balanceInUsd, formatMoney } from '../lib/utils';
import { CRYPTO_PRICES, LNC_RATE_USD } from '../lib/constants';
import { dbUpdateBalance, dbCreateTransaction, dbCreateNotification } from '../lib/sync';
import { notifyUser } from '../lib/telegram-bot';

const PAIRS = [
  { from: 'LNC', to: 'TON', icon: '💎' },
  { from: 'LNC', to: 'USDT', icon: '💵' },
  { from: 'LNC', to: 'BTC', icon: '₿' },
  { from: 'LNC', to: 'ETH', icon: 'Ξ' },
  { from: 'TON', to: 'LNC', icon: '◎' },
  { from: 'TON', to: 'USDT', icon: '💵' },
  { from: 'USDT', to: 'LNC', icon: '◎' },
  { from: 'USDT', to: 'TON', icon: '💎' },
];

function getRate(from: string, to: string): number {
  const fromUsd = CRYPTO_PRICES[from] || 1;
  const toUsd = CRYPTO_PRICES[to] || 1;
...
export default function SwapScreen() {
  ...
  const fee = val * 0.001; // 0.1% swap fee
  ...
  const handleSwap = () => {
    ...
    setTimeout(() => {
      ...
      dbCreateTransaction({
        ...
        from_account_id: fromAcc.id, to_account_id: toAcc.id,
        amount: val, fee, currency: fromCur,
        type: 'transfer', status: 'completed',
        note: `Swap ${fromCur}→${toCur}`,
      }).catch(() => {});
    ...
    <div className="h-full flex flex-col bg-black safe-top">
      ...
      {step === 'input' && (
        <div className="flex-1 px-5 mt-2 overflow-y-auto animate-fade-in pb-8">
          {/* From */}
          <div className="glass-accent rounded-2xl p-4 mb-2">
            <div className="flex items-center justify-between mb-3">
              <span className="text-xs text-white/35 font-medium uppercase">Отдаёте</span>
              <span className="text-xs text-white/25">Баланс: {fromAcc ? fromAcc.balance.toFixed(4) : '0'} {fromCur}</span>
            </div>
            <div className="flex items-center gap-3">
              <input type="number" value={amount} onChange={e => setAmount(e.target.value)}
                placeholder="0.00"
                className="flex-1 bg-transparent text-3xl font-extrabold tabular-nums outline-none text-white" />
              <select value={fromCur} onChange={e => { setFromCur(e.target.value); setAmount(''); }}
                ...
                {['LNC','TON','USDT','BTC','ETH'].map(c => <option key={c} value={c} className="bg-black">{c}</option>)}
              </select>
            </div>
            {/* Quick % buttons */}
            <div className="flex gap-2 mt-3">
              {quickPercents.map(p => (
                <button key={p} onClick={() => {
                  if (fromAcc) setAmount((fromAcc.balance * p / 100).toFixed(6));
                }}
          ...
          <div className="flex justify-center -my-1 relative z-10">
            <button onClick={flipPair}
              className="w-10 h-10 rounded-full bg-white/10 flex items-center justify-center text-lg active:scale-90 active:rotate-180 transition-all duration-300 border border-white/10">
              ⇅
            </button>
          </div>
          ...
          <div className="glass rounded-2xl p-4 mt-2">
            <div className="flex items-center justify-between mb-3">
              ...
              <span className="text-xs text-white/25">Баланс: {toAcc ? toAcc.balance.toFixed(4) : '0'} {toCur}</span>
            </div>
            <div className="flex items-center gap-3">
              <p className="flex-1 text-3xl font-extrabold tabular-nums text-emerald-400">
                {val > 0 ? received.toFixed(6) : '0.00'}
              </p>
              <select value={toCur} onChange={e => setToCur(e.target.value)}
                className="bg-white/10 rounded-xl px-3 py-2 text-sm font-bold outline-none text-white">
                {['LNC','TON','USDT','BTC','ETH'].filter(c => c !== fromCur).map(c => 
          ...
          {/* Rate info */}
          <div className="glass rounded-xl p-3 mt-4 space-y-1.5">
            <div className="flex justify-between text-sm">
              <span className="text-white/35">Курс</span>
              <span className="tabular-nums">1 {fromCur} = {rate.toFixed(6)} {toCur}</span>
            </div>
            <div className="flex justify-between text-sm">
              <span className="text-white/35">Комиссия (0.1%)</span>
              <span className="tabular-nums">{fee.toFixed(6)} {fromCur}</span>
            </div>
            {val > 0 && (
              <div className="flex justify-between text-sm font-bold pt-1 border-t border-white/5">
                <span>Итого спишется</span>
                <span className="tabular-nums">{netSend.toFixed(6)} {fromCur}</span>
              </div>
            )}
          </div>

          {!fromAcc && (
            <div className="glass rounded-xl p-3 mt-3 border border-amber-500/20 flex items-center gap-2">
              <span>⚠️</span>
              <p className="text-xs text-amber-400">Нет счёта в {fromCur}. <button onClick={() => go('open-account')} className="underline">Открыть</button></p>
            </div>
          )}

          <button onClick={() => { haptic('medium'); setStep('confirm'); }}
            disabled={!canSwap}
      ...
      {step === 'confirm' && (
        <div className="flex-1 px-5 mt-4 animate-fade-in">
          ...
          <button onClick={() => setStep('input')} className="btn-ghost w-full mt-2">Отмена</button>
        </div>
      )}
