...
import { HomeIcon, WalletIcon, TrendingUpIcon, BuildingIcon, UserIcon } from './Icons';
    ...
    <nav className="fixed bottom-0 left-0 right-0 z-50 glass-strong safe-bottom">
      <div className="flex items-center justify-around py-2 max-w-lg mx-auto">
        {TABS.map((t, i) => {
          ...
          const Icon = t.icon;
          return (
            <button key={i} onClick={() => { haptic('light'); setTab(i); }}
              className={`flex flex-col items-center gap-1 py-1.5 px-4 rounded-2xl transition-all duration-200 ${isActive ? 'bg-white/[0.06]' : ''}`}>
              <Icon size={20} color={isActive ? '#ffffff' : 'rgba(255,255,255,0.3)'} />
